...
function drawThree() {
    ...
    if (drawIndex >= deck.length) {
        ...
        // ✅ Yeni tur: açılan kartlar da temizlensin
        drawIndex = 0;
        ...
        setTimeout(() => deckEl.classList.remove("flash"), 500); // animasyon bittiğinde kaldır
...
function placeCardOnSeries(index) {
    ...
    if (canPlaceCardOnSeries(index, card)) {
        ...
        const container = document.querySelectorAll('.series-container')[index];
...
// karti masaya ekle
        ...
        if (table[index].length === 13) {
            ...
            setTimeout(() => container.classList.remove("complete"), 1000);
